...
void CScriptEngine::print_stack()
{
	...
	while (lua_getstack(L, level, &l_tDebugInfo))
    {
        lua_getinfo(L, "nSlu", &l_tDebugInfo);
        if (!l_tDebugInfo.name)
        {
            script_log(LuaMessageType::Error, "%2d : [%s] %s(%d) : %s", level, l_tDebugInfo.what, l_tDebugInfo.short_src,
            l_tDebugInfo.currentline, "");
        }
        else if (!xr_strcmp(l_tDebugInfo.what, "C"))
            script_log(LuaMessageType::Error, "%2d : [C  ] %s", level, l_tDebugInfo.name);
        else
        {
            script_log(LuaMessageType::Error, "%2d : [%s] %s(%d) : %s", level, l_tDebugInfo.what, l_tDebugInfo.short_src,
...
void CScriptEngine::lua_error(lua_State* L)
{
	...
	const char *caMessage = lua_isstring(L, -1) ? lua_tostring(L, -1) : "";

	GlobalEnv.ScriptEngine->error_log("LUA error: %s", caMessage);
	GlobalEnv.ScriptEngine->print_stack();

    xrDebug::Fatal(DEBUG_INFO, "LUA error: %s", caMessage);
#else
