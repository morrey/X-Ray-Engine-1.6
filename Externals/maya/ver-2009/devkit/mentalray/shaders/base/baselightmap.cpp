...
typedef struct mib_lightmap_write_param{
    ...
    miInteger   fg_preproc_mode; /* 0 = per vertex, 1 = per pixel, 2 = every other pixel... */
...
extern "C" DLLEXPORT int mib_lightmap_write_version(void) {return(2);}
...
{
	int				i;
	mib_lightmap_write_result *resdata;
	miImg_image			*img;
	miTag				tex_tag;
	miTag				shader_tag;
	miTag				coordshader_tag = miNULLTAG;
	void				*handle;
	miBoolean			success;

    miInteger  fg_mode = *mi_eval_integer(&param->fg_preproc_mode);
	...
	switch (state->type) {
	  case miRAY_LM_VERTEX:
		...
		{
			result->tex.x = result->tex.y = result->tex.z = 0.0f;
		}

		/* Do the "force FG calc" on verticies */
		if (fg_mode == 0)
		{
			miColor tempvalue; /* Never actually used */
			mi_compute_irradiance(&tempvalue, state);
		}
		break;
	  ...
	  case miRAY_LM_MESH:
		...
		resdata = (mib_lightmap_write_result *)arg->vertex_data;

		/* No coordsshader tag? Use 1st texture space */
		if (coordshader_tag == miNULLTAG)
		{
			/* No "coords" shader? Use first texture space */
			for (i=0; i < arg->no_triangles; i++) {
				const miVector *a, *b, *c;
				mi_state_set_pri(state, arg->pri, arg->triangles[i].pri_idx);
				mi_tri_vectors(state, 't', 0, &a, &b, &c);
				
				/* Put vertex coords in there. */
				resdata[arg->triangles[i].a].tex = *a;
				resdata[arg->triangles[i].b].tex = *b;
				resdata[arg->triangles[i].c].tex = *c;
			}
		}


		if (fg_mode > 0 && state->options->finalgather)
		{
			miRay_type old_type = state->type;
			
			/* Need this mode to do the precalc "force fg" calculations */
    ...
    int                 fg_mode)
{
	...
	for (p.y=y_min; p.y <= y_max; p.y++) {
		...
		for (p.x=left_x; p.x <= right_x; p.x++) {
			...
			if (fg_mode) /* Preprocess */
			{
				miColor tempvalue; /* Never actually used */
				if (((((int)p.x) % fg_mode) == 0) &&
					((((int)p.y) % fg_mode) == 0))
						mi_compute_irradiance(&tempvalue, state);
			}
			else
