...
extern "C" DLLEXPORT int mib_fast_occlusion_version(void) {return(1);}


extern "C" DLLEXPORT miBoolean mib_fast_occlusion(
	miColor     *result,
	miState     *state,
	struct mib_fast_occlusion_p *paras)
{
	miScalar output;
	...
	miBoolean ret_type	= *mi_eval_integer(&paras->return_type);
	miScalar  spread	= *mi_eval_scalar(&paras->spread);
	miScalar  spread_exp	= *mi_eval_scalar(&paras->spread_exp);
	miBoolean occ_alpha	= *mi_eval_boolean(&paras->occlusion_in_alpha);
	miBoolean reflecto	= *mi_eval_boolean(&paras->reflective);
	miInteger interp	= *mi_eval_integer(&paras->interp);

	miVector  orig_normal = state->normal, orig_dir = state->dir;

	miVector  bent_normal;
	struct miAmboccl_options amopts;
	miAMBOCCL_DEFAULT(&amopts, state);

	if (reflecto)
	    mi_reflection_dir(&amopts.axis, state);

	if (samples > 0)    amopts.rays		= samples;
	if (clipdist > 0)   amopts.maxdist	= clipdist;
	if (spread > 0)	    amopts.spread	= spread;
	if (spread_exp > 0) amopts.spread_exp	= spread_exp;
	if (interp > 0)	    amopts.cache_points	= interp;

	/* the computed bent normal is in world space */
	output = 1.0f - mi_ambient_occlusion (state, &amopts, &bent_normal);
	...
	switch (ret_type) {
		...
		default:  /* (also the default for out-of-bounds values) */
		{
			...
			if (ret_type == -1)
				...
				/* If the color shaders use the normal....
					give them the bent one... */
				state->normal = bent_normal;
				state->dir    = bent_normal;
			}
			if (output == 0.0)
				*result = *mi_eval_color(&paras->dark);
			else if (output >= 1.0)
		...
		case 1: /* Sampled environment */
		{
			...
			/* This is unlike the tracing version, we just do ONE 
			lookup in the bent normal direction */
			mi_trace_environment(&env_total, state, &bent_normal);

			result->r = drk.r + (br.r * env_total.r * output);
			result->g = drk.g + (br.g * env_total.g * output);
			result->b = drk.b + (br.b * env_total.b * output);
			if (occ_alpha)
				result->a = output;
			else
				result->a = 1.0;
		...
		case 4: /* Bent normals, object */
		{
			...
			if (ret_type == 2)
				retn = bent_normal;	/* world already */
			if (ret_type == 3)  {
				miVector int_vector;
				mi_normal_from_world(state, &int_vector, &bent_normal);
				mi_normal_to_camera(state, &retn, &int_vector);
				mi_vector_normalize (&retn);
			}
			if (ret_type == 4)  {
				miVector int_vector;
				mi_normal_from_world(state, &int_vector, &bent_normal);
				mi_normal_to_object(state, &retn, &int_vector);
				mi_vector_normalize (&retn);
			}
