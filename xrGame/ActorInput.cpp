...
{
	return IsFocused() && (cam_active == eacFirstEye || (cam_active == eacLookAt && m_holder->allowWeapon() && m_holder && m_holder->HUDView()));
}
...
void CActor::actor_kick()
{
	...
	if (O)
	{
		...
		if (mass_f > 5)
		{
			hit.boneID = 0;  // ïîêà íå ÿñíî, êóäà ëóø÷å ÃÃ óäàðèòü (â íîãó íàäî?)
			this->Hit(&hit); // ñèëà äåéñòâèÿ ðàâíà ñèëå ïðîòèâîäåéñòâèÿ
		}
	}
}
